...
st.markdown(
    ...
    <style>
      ...
      [data-testid="stMetric"] { background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); padding: 0.85rem 0.95rem; border-radius: 14px; }
      [data-testid="stMetric"] [data-testid="stMetricLabel"] { opacity: 0.85; }
...
def load_country_boundaries():
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(response.content)) as zip_file:
            ...
            temp_dir = tempfile.mkdtemp(prefix="ne_countries_")
...
def _first_index_from_selection(selection_dict):
    if not selection_dict:
        return None
    indices = selection_dict.get("indices")
    if indices is None:
        return None
    ...
    def _extract_idx(v):
        ...
        if isinstance(v, dict):
            ...
            if "indices" in v:
                r = _extract_idx(v.get("indices"))
                if r is not None:
                    return r

            skip_keys = {"object", "picked", "layer", "id", "info", "properties"}
            for k, vv in v.items():
                if k in skip_keys:
                    continue
                r = _extract_idx(vv)
    ...
    return _extract_idx(indices)


def _view_state_for_location(location, default_view_state):
    if not location:
        return default_view_state

    try:
        if "lat" in location and "lon" in location and pd.notnull(location.get("lat")) and pd.notnull(location.get("lon")):
            return pdk.ViewState(
                latitude=float(location["lat"]),
                longitude=float(location["lon"]),
                zoom=float(location.get("zoom", 5.2)),
                pitch=0,
            )
        ...
        if geom:
            ...
            zoom_lon = float(np.log2(360.0 / width))
            zoom_lat = float(np.log2(170.0 / height))
            zoom = float(np.clip(min(zoom_lon, zoom_lat) - 0.6, 1.0, 8.0))
            return pdk.ViewState(
...
def _selection_layers(display_data, is_point_mode, is_grid_mode):
    idx = st.session_state.get("selected_index")
    if idx is None:
        return []
    if display_data is None or display_data.empty:
        return []
    if not (0 <= int(idx) < len(display_data)):
        return []

    row = display_data.iloc[int(idx)].to_dict()

    if is_grid_mode or is_point_mode:
        selected_df = pd.DataFrame([row])
        return [
            pdk.Layer(
                "ScatterplotLayer",
                selected_df,
...
def render_view_summary(display_data, display_mode):
    """Render summary statistics for the current view in the side panel."""
    st.markdown(f"### 🌍 {display_mode} Overview")
    ...
    # Summary Metrics
    total_locs = len(display_data)
    
    if "heat_hours" in display_data.columns:
        avg_heat = float(display_data["heat_hours"].mean())
        max_heat = float(display_data["heat_hours"].max())

        c1, c2 = st.columns(2)
        c1.metric("Locations", f"{total_locs:,}")
        c2.metric("Avg Heat Hours", f"{avg_heat:,.0f}")

        if "name" in display_data.columns:
            top_n = min(10, len(display_data))
            top = display_data.sort_values("heat_hours", ascending=False).head(top_n)
            st.markdown("#### Top Hotspots")
            show_cols = [c for c in ["name", "continent", "country", "heat_display"] if c in top.columns]
            if show_cols:
                st.dataframe(top[show_cols], use_container_width=True, hide_index=True)

    st.info("Select a location on the map (or use the picker below) to see detailed analysis.")


def render_location_details(location_data, metric_label=None):
    """Render detailed metrics for a selected location."""
    if not location_data:
        st.warning("No location selected.")
        return

    name = location_data.get('name')
    if name is None and ('lat' in location_data) and ('lon' in location_data):
        name = "Selected Grid Cell"

    st.markdown(f"## 📍 {name or 'Unknown Location'}")
    if location_data.get('country'):
        st.caption(f"{location_data.get('country', '')}")

    st.divider()
    
    # Primary Metric
    heat_hours = location_data.get('heat_hours', 0)
    st.metric(metric_label or "Heat Impact", f"{heat_hours:,.0f}")
    
    # Context Metrics
    pop = location_data.get('population', 0)
    if pop:
        st.metric("Population Exposed", f"{pop:,.0f}")

    if ('lat' in location_data) and ('lon' in location_data) and pd.notnull(location_data.get('lat')) and pd.notnull(location_data.get('lon')):
        st.metric("Coordinates", f"{float(location_data.get('lat')):.3f}, {float(location_data.get('lon')):.3f}")
    
    # Insight / Analysis (Placeholder for now)
    st.markdown(f"<div class='quiet'>This location experiences {heat_hours:,.0f} under the selected configuration.</div>", unsafe_allow_html=True)
...
def main():
    ...
    with tab_dashboard:
        ...
        context_key = (scenario, metric_type, metric, bool(person_hours), timescale, int(year), display_mode)
        if st.session_state.selection_context != context_key:
            st.session_state.selection_context = context_key
            st.session_state.selected_location = None
            st.session_state.selected_index = None
        ...
        # ---------------- LAYOUT: MAP & CONTEXT ----------------
        col_map, col_details = st.columns([0.62, 0.38], gap="medium")
        ...
        with col_map:
            ...
            if st.session_state.selected_index is not None and not display_data.empty:
                try:
                    st.session_state.selected_location = display_data.iloc[int(st.session_state.selected_index)].to_dict()
                except Exception:
                    st.session_state.selected_location = None
                    st.session_state.selected_index = None

            default_view_state = pdk.ViewState(latitude=9.0, longitude=5.0, zoom=3.0, pitch=0)
            view_state = _view_state_for_location(st.session_state.selected_location, default_view_state)
            ...
            # Simplified Tooltip for Map (since we have side panel)
            # Keeping it for quick hover context
            if is_grid_mode:
                tooltip = {"html": f"<b>Val:</b> {{heat_hours}}", "style": {"color": "white", "backgroundColor": "#333"}}
            else:
                tooltip = {"html": f"<b>{{name}}</b><br/>{metric_label}: {{heat_display}}", "style": {"color": "white", "backgroundColor": "#333"}}
            
            layers = layers + _selection_layers(display_data, is_point_mode, is_grid_mode)
            ...
            # Render Map with Selection Event (Experimental)
            try:
                # Attempt to use selection state if available in this streamlit version
                selection = st.pydeck_chart(
                    deck, 
                    use_container_width=True,
                    on_select="rerun", # Rerun to capture selection
                    selection_mode="single-object"
                )
                
                if selection.selection:
                    idx = _first_index_from_selection(selection.selection)
                    if idx is not None and not display_data.empty and 0 <= int(idx) < len(display_data):
                        st.session_state.selected_index = int(idx)
                        st.session_state.selected_location = display_data.iloc[int(idx)].to_dict()
        ...
        with col_details:
            ...
            if (not is_grid_mode) and (not display_data.empty) and ("name" in display_data.columns):
                labels = display_data["name"].fillna("Unknown").astype(str)
                options = ["—"] + list(range(len(display_data)))
                default_idx = 0
                if st.session_state.selected_index is not None and 0 <= int(st.session_state.selected_index) < len(display_data):
                    default_idx = int(st.session_state.selected_index) + 1
                picked = st.selectbox(
                    "Jump to location",
                    options=options,
                    index=default_idx,
                    format_func=lambda x: "—" if x == "—" else labels.iloc[int(x)],
                )
                if picked != "—" and int(picked) != int(st.session_state.get("selected_index") or -1):
                    st.session_state.selected_index = int(picked)
                    st.session_state.selected_location = display_data.iloc[int(picked)].to_dict()
                    st.rerun()

            if st.session_state.selected_location:
                loc = st.session_state.selected_location

                if "heat_hours" in display_data.columns and not display_data.empty and "heat_hours" in loc:
                    try:
                        values = display_data["heat_hours"].astype(float)
                        rank = int(values.rank(method="min", ascending=False).iloc[int(st.session_state.selected_index)]) if st.session_state.selected_index is not None else None
                        total = int(len(values))
                        percentile = float((values < float(loc.get("heat_hours", 0))).mean() * 100.0)

                        c1, c2 = st.columns(2)
                        c1.metric("Rank", f"{rank:,}/{total:,}" if rank is not None else "—")
                        c2.metric("Percentile", f"{percentile:,.1f}%")
                    except Exception:
                        pass

                tabs = st.tabs(["Overview", "Context", "Compare"])
                with tabs[0]:
                    render_location_details(loc, metric_label=metric_label)
                ...
                with tabs[1]:
                    if "continent" in loc and loc.get("continent"):
                        st.metric("Continent", str(loc.get("continent")))
                    if "country" in loc and loc.get("country"):
                        st.metric("Country", str(loc.get("country")))
                    if "lat" in loc and "lon" in loc and pd.notnull(loc.get("lat")) and pd.notnull(loc.get("lon")):
                        st.metric("Coordinates", f"{float(loc.get('lat')):.3f}, {float(loc.get('lon')):.3f}")

                with tabs[2]:
                    if not display_data.empty and "heat_hours" in display_data.columns:
                        peer = display_data
                        if "continent" in loc and "continent" in display_data.columns and pd.notnull(loc.get("continent")):
                            peer = peer[peer["continent"] == loc.get("continent")]
                        elif "country" in loc and "country" in display_data.columns and pd.notnull(loc.get("country")):
                            peer = peer[peer["country"] == loc.get("country")]
                        peer = peer.sort_values("heat_hours", ascending=False).head(10)
                        show_cols = [c for c in ["name", "continent", "country", "heat_display"] if c in peer.columns]
                        if show_cols and len(peer) > 0:
                            st.dataframe(peer[show_cols], use_container_width=True, hide_index=True)
                        else:
                            st.info("No peer comparison available.")
            else:
                render_view_summary(display_data, display_mode)
    ...
    with tab_docs:
        ...
        **2. WBGT (Wet Bulb Globe Temperature  - Units °C)**
        ...
        - **References**: 
        (i) Brimicombe et al., 2023: https://doi.org/10.1029/2022GH000701, 
