...
def main():
    ...
    display_mode = st.sidebar.radio(
        ...
        help="All Cities: ~4K cities with pop>100K | Priority Cities: 3 African cities | Countries: All countries | Continents: Aggregated by continent"
    )
    ...
    with st.spinner("Computing metrics..."):
        is_point_mode = display_mode in ["All Cities", "Priority Cities"]
        
        if display_mode == "All Cities":
        ...
        elif display_mode == "Priority Cities":
            # Use hardcoded priority cities
            city_data = []
            ...
            display_data = pd.DataFrame(city_data)
            st.sidebar.info("Showing 3 priority African cities")
        
        elif display_mode == "Countries":
            # All countries with individual heat values
            if world_gdf is not None:
                ...
                st.sidebar.error("Could not load country boundaries")
                display_data = pd.DataFrame()
    ...
    # 1. Filter out zero values to save resources and reduce visual clutter
    ...
    tooltip = {
        "html": f"""
            <div style='padding: 8px; font-family: Arial, sans-serif;'>
                ...
                {'<span style=\'color: #aaa;\'>{{country}}</span><br/><br/>' if is_point_mode else ''}
                {'<span style=\'color: #aaa;\'>{{continent}}</span><br/><br/>' if display_mode == 'Countries' else ''}
    ...
    if not display_data.empty:
        if is_point_mode:
            ...
            display_df = display_data[cols].copy()
            new_cols = ["Location", metric_label, "Population"]
            if "continent" in display_data.columns:
                new_cols.insert(1, "Continent")
            display_df.columns = new_cols
            
            export_cols = ["name", "heat_hours", "population"]
            if "continent" in display_data.columns:
                export_cols.insert(1, "continent")
            export_df = display_data[export_cols].copy()
            
            export_header = ["Location", metric_label, "Population"]
            if "continent" in display_data.columns:
                export_header.insert(1, "Continent")
            export_df.columns = export_header
